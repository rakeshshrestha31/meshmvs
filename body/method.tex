...
\siyu{The intuition and basic operations of graph-cnn-based mesh deformation should be given first}
...
\siyu{How does the previous approach obtain the features. Any limitations and problems of the previous approaches?}
\rakesh{comparison with previous approach added}
\cite{yao2020front2back} demonstrate that using intermediate, image-centric 2.5D representations instead of directly generating 3D shapes in global frame from raw 2D images~\cite{wang2018pixel2mesh,wen2019pixel2mesh++} can improve 3D reconstruction quality.
...
To constrain the mesh predicted by each GCN block (P) to resemble the ground truth (Q) we use
Chamfer distance~\cite{fan2017point} between nearest neighbor mesh vertices $\Lambda$ i.e.
$\mathcal{L}_{\text{chamfer}}(\text{P}, \text{Q}) = |\text{\text{P}}|^{-1} \sum_{(p, q) \in \Lambda_{\text{P},\text{Q}}}{||p-q||^{2}} + |\text{Q}|^{-1} \sum_{(q, p) \in \Lambda_{\text{Q},\text{P}}}{||q-p||^{2}}$ to learn to deform vertices to correct positions
along with surface normal loss between vertex normals~\cite{wang2018pixel2mesh}
$\mathcal{L}_{\text{normal}}(\text{P}, \text{Q}) = -|\text{P}|^{-1} \sum_{(p, q) \in \Lambda_{\text{P},\text{Q}}}{|u_p \cdot u_q|} - |\text{Q}|^{-1} \sum_{(q, p) \in \Lambda_{\text{Q},\text{P}}}{|u_q \cdot u_p|}$ to learn local higher order surface properties.
Additional regularization in the form of edge length loss
$\mathcal{L}_{\text{edge}}(\text{V}, \text{E}) = \frac{1}{|E|} \sum_{(v,v') \in E}{||v - v'||^2}$
is also applied
which penalizes long edges resulting in flying vertices~\cite{wang2018pixel2mesh} for visually appealing results.
...
\paragraph{Voxel loss} Binary cross-entropy loss between the predicted voxel occupancy probabilities and the ground truth occupancies is used as voxel loss to supervise the voxel predictions to constrain the coarse initial shapes~\cite{gkioxari2019meshrcnn}
...
\rakesh{Additional discussions added, original work using the losses cited for further reference}
